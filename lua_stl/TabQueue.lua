--[[
	说明:
		...
		每次PopFront(), PushFront(), 都会触发lua table 的'自身重新排序机制':
			PushFront() = _G.table.insert()
			PopFront() = _G.table.remove()
		(这个功能不需要你自己造, 直接使用_G.table.remove() 即可, 
		 lua table会自己'删除/调整'下标指定的元素, 并触发重新排列)

		重新排序时, 直接使用_G.table.sort()
...
function TabQueue:Sort()
	--(直接使用_G.table.sort()进行排序)
	local Type = self.Type
	local Sort = _G.table.sort
	local Body = self.body
	local Pos = self.Pos
	if Type == "anything"  or Pos <= 1 then
		return
	end
	Sort(Body)
end

--洗牌乱序, 打乱TabQueue 原来的顺序, 随机排序(anything 也可以乱序)
