...
static const struct luaL_Reg getRandom[] = {
...
ps:
	luaL_register()//lua5.1
	luaL_newlib()//lua5.3 方法1
	luaL_newlibtable()+luaL_setfuncs()//lua5.3 方法2
	版本差异较大, 详情自己看lauxlib.h 头文件, 看看里面是怎样定义的, 版本差异较大!!
*/
int luaopen_getRandom (lua_State * L)
{
	//lua 5.1 专用的注册函数
	luaL_register (L, "getRandom", getRandom);

	//lua 5.3 专用的注册函数
	//luaL_newlib(L, getRandom);//lua 5.3 设置函数table 的方法1

	//luaL_newlibtable(L, getRandom);//lua 5.3 设置函数table 的方法2
	//luaL_setfuncs(L, getRandom, 0);

	//将本table 模块注册进lua 机的全局_G table 中(慎重, 应该避免污染_G, 还是使用local t = require "getRandom" 比较好)
	//lua_setglobal(L, "getRandom");
	return 1;
}
